...
## WhatsApp quick replies — structured follow-up options, not text lists

Every agent's **Guardrails** tab has a **WhatsApp quick replies** section. This solves
a specific problem: instead of the model listing follow-up options as numbered text
inside its reply (which your WhatsApp integration then has to parse or just display as
plain text), every response includes a `quickReplies` object — `{"followup1": "...",
"followup2": "...", "followup3": "..."}` — ready to map directly onto native WhatsApp
quick-reply buttons.

**The guarantee, enforced by the platform, not just prompted to the model**:
- Always exactly 3 keys: `followup1`, `followup2`, `followup3`.
- **`followup3` is always the exact text you configured** (default `"Enroll Now"`,
  fully customizable — e.g. `"Buy Now"`, `"Book Now"`, `"Talk to Sales"`) — generated by
  the platform's own code, never by the model, so it can never be dropped, reworded, or
  forgotten on a bad turn.
- **`followup1`/`followup2` respect your configured character limit** (default 20,
  matching WhatsApp's quick-reply button constraint). If the model returns something
  longer, it's fit to the limit at a **word boundary** — never cut mid-word — so you get
  a clean
  shorter phrase, not a broken fragment like `"Check pricing for fa"`. If even the first
  word doesn't fit, that option is dropped in favor of the safe fallback rather than
  showing something unusable. The system prompt also pushes the model hard to produce
  genuinely short, complete phrases in the first place (with concrete good/bad examples),
  so this fallback should rarely trigger in practice — it exists as a safety net, not the
  primary mechanism.
- If the model returns fewer than 2 usable dynamic options (or fails to follow the format
  at all), safe fallback text fills the gap rather than the response ever having fewer
  than 3 items.

**How it works under the hood**: the first 2 options come from the model — enabling this
switches the agent to a structured JSON response contract (`{"message": "...", "options":
["...", "..."]}`) instead of plain text, in the same LLM call, no added latency or cost.
The 3rd is appended by platform code afterward. Works for both catalog-based and generic
agents — for catalog agents, the signal can arrive alongside a real product
recommendation in the same response.

**Approved options pool (recommended over free text)**: instead of trusting the model
to invent short, sensible option text on the fly, you can pre-write a curated list of
options in the Guardrails tab. Once a pool has at least one entry, the model is
instructed to **select exactly 2 items from that exact list** — copying the text
verbatim — rather than generating anything itself. This eliminates the length/sense
problem at the root rather than patching it after the fact: every pool entry is
validated to fit your character limit *at save time* (a too-long entry is rejected
with a clear error before it's ever used, not discovered mid-conversation), and every
entry reads as a complete, sensible phrase because you wrote it that way.

Server-side, whatever the model returns is checked against the pool (case/whitespace-
insensitive exact match); anything that isn't a genuine pool member — including a fully
hallucinated option — is discarded and silently replaced with another real, unused pool
entry. Verified this holds even when a model is deliberately fed instructions to
misbehave: the final result is always drawn from real, pre-approved text, never
model-improvised prose, whenever the pool has enough entries to draw from.

Leave the pool empty and quick replies fall back to free-text generation (the model
writes its own short options, fit to the character limit at a word boundary as a safety
net) — still functional, just less predictable than a curated pool.

**Interaction with the conversion signal**: `quickReplies` is omitted entirely on a turn
where the conversion signal fires — once a customer is being handed off to another
workflow, follow-up buttons for continuing the conversation don't make sense.

**One thing this doesn't do**: it doesn't talk to WhatsApp's API directly — this platform
