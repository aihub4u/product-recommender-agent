...
plain text), every response includes a `quickReplies` array — 3 short strings ready to
render as native WhatsApp quick-reply buttons.

**The guarantee, enforced by the platform, not just prompted to the model**:
- Always exactly 3 items.
- The **last item is always the exact text you configured** (default `"Enroll Now"`,
...
- **Every item respects your configured character limit** (default 20, matching
  WhatsApp's quick-reply button constraint). If the model returns something longer, it's
  fit to the limit at a **word boundary** — never cut mid-word — so you get a clean
